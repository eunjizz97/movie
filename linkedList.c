...
typedef struct node{
	int index; //index of the node / node의 index 
	void* obj; //object data / obj 데이터 포인터 
	void* next; //pointer to the next / 다음으로의 포인터 

} node_t;		//이것을 node_t 라고 정의 

//Inner functions (cannot used at the outside of this file)
node_t* genNode(void)
{
	node_t* ndPtr;	//node 포인터를 만듦 
	
	//allocate memory for creating the node
	ndPtr = (node_t*)malloc(sizeof(node_t));	//node_t 크기만큼 메모리 할당 
	if (ndPtr != NULL)							//ndPtr 포인터가 어딘가를 가리키고 있으면 
	{
		ndPtr->next = NULL;						//next 가 0을 가리키게 함 
		ndPtr->obj = NULL;						//obj 도 0을 가리키게 함 
	}
